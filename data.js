...
async function getSql() {
  const urls = getUrls();
  if (!urls.length) throw new Error('Database connection string is missing');
...
export default async function handler(req, res) {
  try {
    ...
    if (req.method === 'GET') {
      ...
      return res.status(200).json(rows[0]?.data || initialData);
    }
    ...
    if (req.method === 'DELETE') {
      await sql`
        UPDATE app_state
        SET data = ${JSON.stringify(initialData)}::jsonb, updated_at = NOW()
        WHERE id = 1
      `;
      return res.status(200).json({ ok: true });
    }
